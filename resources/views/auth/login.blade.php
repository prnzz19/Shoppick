...
<div class="w-full max-w-md">
    ...
    <div class="card w-full p-6 sm:p-8">
        ...
        @if(session('authentication_error'))<div class="mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700" role="alert">{{ session('authentication_error') }}</div>@endif
        ...
        @else
            <button type="button" disabled class="btn-outline flex w-full cursor-not-allowed items-center justify-center gap-3 opacity-55"><span class="text-lg font-bold text-brand-600">G</span>Continue with Google</button>
            <p class="mt-2 text-center text-xs text-slate-400">Google sign-in is currently unavailable.</p>
        @endif
