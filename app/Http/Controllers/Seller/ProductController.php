...
class ProductController extends Controller
{
    ...
    public function store(Request $request) { $data = $this->validateProduct($request); $data['store_id'] = $request->user()->store->id; $this->products->create($data); return redirect()->route('seller.products.index')->with('success', 'Product created.'); }
    ...
    public function update(Request $request, Product $product) { $this->own($product); $data = $this->validateProduct($request, $product); $data['store_id'] = $request->user()->store->id; $this->products->update($product, $data); return redirect()->route('seller.products.index')->with('success', 'Product updated.'); }
